...
function diasDropdown(nombre, campo, seleccionados, label, prefix=''){
  ...
  const opts = DIAS_SEMANA.map(d=>{
    ...
    return '<label>'+
      '<input type="checkbox" value="'+d.v+'" '+chk+
      ' data-campo="'+campo+'" data-prefix="'+esc(prefix)+'"> '+
      d.l+
      '</label>';
  }).join('');

  return '<details class="dias-dropdown" id="'+id+'">'+
    '<summary>'+esc(label)+(count?' ('+count+')':'')+'</summary>'+
    '<div class="dias-panel">'+opts+'</div>'+
    '</details>';
}
...
function conteoDiasDropdown(nombre, campo, conteo, label, prefix=''){
  ...
  const total = DIAS_SEMANA.reduce(
    (s,d)=>s+(Number(conteo[d.v])||0),
    0
  );
  ...
  const filas = DIAS_SEMANA.map(d=>{
    ...
    return '<label style="justify-content:space-between;gap:10px">'+
      '<span>'+d.l+'</span>'+
      '<input type="number" min="0" step="1" style="width:52px" value="'+val+'" '+
      'data-campo="'+campo+'" data-dia="'+d.v+'" data-prefix="'+esc(prefix)+'">'+
      '</label>';
  }).join('');

  return '<details class="dias-dropdown">'+
    '<summary>'+esc(label)+(total?' ('+total+')':'')+'</summary>'+
    '<div class="dias-panel">'+filas+'</div>'+
    '</details>';
}
...
function chPoblateFilters(){
  ...
  const empOpts =
    ...
    chEmpleados.map(e=>
      '<option value="'+esc(e.nombre)+'">'+
      esc(e.nombre_display)+
      '</option>'
    ).join('');
...
document.getElementById(
  ...
  async ()=>{
    ...
    if(!texto.trim()){
      alert(
        'Subí un archivo o pegá los datos primero.'
      );
...
function renderPreviewEmpleados(data, mes){
  ...
  data.empleados.forEach(n=>{
    chAjustesPendientes[n] =
      data.ajustes[n] || {
        faltas:[],
        feriados:[],
        vacaciones:{},
  ...
  let html =
    '<div class="alerta-card info" style="margin-bottom:16px">'+
    'Se encontraron <strong>'+
    data.total_marcas+
    '</strong> marcas para <strong>'+
  ...
  cont.querySelectorAll(
    'input[type=number][data-campo=vacaciones]'
  ).forEach(inp=>{
...
async function renderDetalle(){
  ...
  try{
    ...
    if(!rows.length){
      cont.innerHTML =
        emptyState(
          'No hay fichadas para este filtro.'
        );
    ...
    let html =
      ...
      '<th>Empleado</th>'+
      '<th>Fecha</th>'+
      '<th style="text-align:right">Marca 1</th>'+
      '<th style="text-align:right">Marca 2</th>'+
      '<th style="text-align:right">Marca 3</th>'+
      '<th style="text-align:right">Marca 4</th>'+
      '<th style="text-align:right">Horas (bruto)</th>'+
      '<th>Estado</th>'+
      '<th style="text-align:right">A liquidar</th>'+
      '</tr>'+
      '</thead>'+
      '<tbody>';
...
function renderResumenSnapshot(data){
  ...
  let html =
    '<div class="alerta-card ok" style="margin-bottom:16px">'+
...
function renderResumenVivo(data, mes){
  ...
  const ok =
    data.resumenes.filter(
      r=>r.estado==='OK'
    ).length;

  const faltan =
    data.resumenes.filter(
      r=>r.estado==='Faltan horas'
    ).length;

  const errores =
    data.resumenes.reduce(
      (s,r)=>s+r.dias_con_error,
      0
    );
  ...
  let html =
    ...
    '<div class="card">'+
    '<div class="label">Empleados OK</div>'+
    '<div class="value">'+
    ok+
    '</div>'+
    '</div>'+
    ...
    '<div class="card">'+
    '<div class="label">Con horas faltantes</div>'+
    '<div class="value">'+
    faltan+
    '</div>'+
    '</div>'+
    ...
    '<div class="card">'+
    '<div class="label">Días con error</div>'+
    '<div class="value">'+
    errores+
    '</div>'+
    '</div>'+
  ...
  const conAdv =
    data.resumenes.filter(
      r=>
        r.advertencia &&
        r.advertencia.length>0
    );

  if(conAdv.length){
    html+=
      '<div style="margin-top:24px;padding-top:18px;border-top:1px solid var(--gray100)">'+
      '<h2 style="color:var(--error);font-size:1.05rem;margin-bottom:14px">'+
      '⚠ Advertencias'+
      '</h2>';
  ...
  async ()=>{
    ...
    if(
      !confirm(
        '¿Cerrar '+
        labelMes(mes)+
        '? No se va a poder reimportar ni modificar ajustes de ese mes.'
      )
    ) return;
...
if(ES_ADMIN){
  ...
  if(btnAbrir){
    btnAbrir.addEventListener(
      ...
      async ()=>{
        const mes =
          prompt(
            '¿Qué mes querés abrir? (formato YYYY-MM)'
          );
        ...
        if(
          !confirm(
            '¿Abrir '+
            mes+
            '? Los datos de cierre se van a borrar.'
          )
        ) return;
  ...
  const btnConfirmarLimpiar =
    document.getElementById(
      'chBtnConfirmarLimpiarMes'
    );

  const btnCancelarLimpiar =
    document.getElementById(
      'chBtnCancelarLimpiarMes'
    );
  ...
  if(btnLimpiar){
    btnLimpiar.addEventListener(
      ...
      ()=>{
        ...
        if(!abiertos.length){
          alert(
            'No hay meses abiertos con datos para limpiar.'
          );

          return;
        }

        panelLimpiar.style.display =
          'block';
  ...
  if(btnCancelarLimpiar){
    btnCancelarLimpiar.addEventListener(
      'click',
      ()=>{
        panelLimpiar.style.display =
          'none';
  ...
  if(btnConfirmarLimpiar){
    btnConfirmarLimpiar.addEventListener(
      'click',
      async ()=>{
        const mes =
          selectLimpiar.value;
        ...
        if(!infoMes){
          alert(
            'Ese mes ya no tiene datos para limpiar. Actualizá la página e intentá nuevamente.'
          );
        ...
        if(infoMes.cerrado){
          alert(
            'Ese mes está cerrado. Primero tenés que abrirlo.'
          );
        ...
        btnConfirmarLimpiar.disabled =
          true;

        btnConfirmarLimpiar.textContent =
          'Limpiando...';
        ...
        try{
          ...
          alert(
            labelMes(mes)+
            ' se limpió correctamente. Ya podés volver a importar el archivo del reloj.'
          );
...
async function renderEvolucion(){
  ...
  try{
    ...
    if(!data.meses.length){
      cont.innerHTML =
        emptyState(
          'No hay datos todavía. Importá fichadas primero.'
        );
    ...
    if(empFiltro){
      ...
      if(emp){
        ...
        if(liqs.length){
          ...
          liqs.forEach(l=>{
            html+=
              ...
              (
                ES_ADMIN
                  ? '<button class="btn btn-danger btn-sm" onclick="eliminarLiquidacion('+
                    l.id+
                    ')">🗑</button>'
                  : ''
              )+
          ...
          html +=
            '<div style="margin-top:10px;font-size:12.5px;color:var(--gray600)">'+
            'Total liquidado: '+
        ...
        html +=
          ...
          '<input type="date" id="chLiqFecha" value="'+
          todayISO()+
          '">'+
          '</div>'+

          '<div>'+
          '<label class="field-label">Horas a liquidar</label>'+
          '<input type="number" id="chLiqMonto" min="0" step="0.01" style="width:100px" placeholder="0,00">'+
          '</div>'+

          '<div style="flex:1">'+
          '<label class="field-label">Comentario (opcional)</label>'+
          '<input type="text" id="chLiqComentario" style="width:100%" placeholder="ej. Pago en efectivo">'+
          '</div>'+

          '<div style="margin-top:22px">'+
          '<button class="btn btn-primary btn-sm" id="chBtnLiquidar">'+
...
function buildBarChartCH(points){
  ...
  return '<svg '+
    'viewBox="0 0 '+w+' '+h+'" '+
    'style="width:100%;max-width:'+w+'px;height:'+h+'px;margin:16px 0">'+
...
async function renderConfig(){
  ...
  try{
    ...
    let html =
      ...
      '<th>Alias (nombre en reloj)</th>'+
      '<th title="Media jornada">½ Jornada</th>'+
    ...
    configLocal.forEach(
      (e,i)=>{
        html+=
          ...
          '<input type="text" '+
          'data-idx="'+i+'" '+
          'data-campo="nombre_reloj" '+
          'value="'+
          esc(
            e.nombre_reloj||''
          )+
          '" '+
          'placeholder="Como aparece en el reloj..." '+
          'style="width:220px">'+
          '</td>'+
          ...
          '<input type="checkbox" '+
          'data-idx="'+i+'" '+
          'data-campo="sin_descuento_descanso"'+
          (
            e.sin_descuento_descanso
              ? ' checked'
              : ''
          )+
          '>'+
...
const btnCancelarEmpleado =
  document.getElementById(
    'chBtnCancelarEmpleado'
  );

const btnCrearEmpleado =
  document.getElementById(
    'chBtnCrearEmpleado'
  );

if(btnNuevoEmpleado){
  btnNuevoEmpleado.addEventListener(
    'click',
    ()=>{
      formNuevoEmpleado.style.display =
        'block';
...
if(btnCrearEmpleado){
  btnCrearEmpleado.addEventListener(
    'click',
    async ()=>{
      ...
      if(!nombre_reloj){
        alert(
          'Ingresá el alias/nombre que figura en el reloj.'
        );

        return;
      }

      btnCrearEmpleado.disabled =
        true;

      btnCrearEmpleado.textContent =
        'Creando...';
  ...
  async ()=>{
    ...
    try{
      ...
      if(
        ...
        alert(
          'Empleados unificados correctamente:\n\n'+
          detalle
        );
      }
